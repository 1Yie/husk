...
| 4 | Agent loop | `crates/agent-kernel/src/engine.rs`, `session.rs`, `events.rs` | `AgentState` machine runs prompt → stream → tool → inject → re-sample **headless via MockProvider in `cargo test -p agent-kernel`**; budgets enforced |
