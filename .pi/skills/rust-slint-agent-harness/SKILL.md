...
| 3 | Tools + registry | `crates/agent-kernel/src/tools/*` | `smart_read` (outline/range/search), `list_dir`, `grep`, `fuzzy_patch`, `apply_patch`, `bash` dispatch by name with JSON schemas; match-tiering + content_hash drift check; output truncated per budget table |
...
| 9 | Plugin system | `crates/agent-plugin/src/*` + kernel `hooks.rs`/`commands.rs` | `PluginManager` routes all four extension points; WASM sandbox enforces manifest permissions; full MCP lifecycle (initialize→tools/list→list_changed); consent dialog round-trip |
...
- **`references/native-tools.md`** — in-process file IO that beats script harnesses: `smart_read` (tree-sitter outline / numbered range / search, content_hash), `fuzzy_patch` (search-and-replace blocks, 4-tier match pipeline, hash-drift guard), ~85% token savings vs cat+rewrite, end-to-end edit loop.
