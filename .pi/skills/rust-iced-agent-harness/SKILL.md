...
- **`references/native-tools.md`** — in-process primitives that beat script harnesses: `smart_read`/`symbol_outline` (tree-sitter), `fuzzy_patch` (4-tier match + hash guard), `smart_grep` (scope-annotated), `find_references_lite`, `pty_session` (interaction detection), `smart_test_runner` (failure-only filter), `linter_auto_fix`, `undo_hunk`, `fast_semantic_search`, `convention_distill`, desktop senses; ~85% token savings, 3-phase rollout.
